...
import { useOnWindowResize } from "@/components/hooks/useOnWindowResize"
...
const ScrollButton = ({ icon, onClick, disabled }: ScrollButtonProps) => {
    ...
    React.useEffect(() => {
        if (disabled) clearInterval(intervalRef.current as NodeJS.Timeout)
    }, [disabled])
        ...
        >
            <div
                ...
                className={cx(
                    "flex h-full",
                    enableLegendSlider ?
                        hasScroll && "snap-mandatory items-center overflow-auto pr-12 pl-4 scrollbar-none [&::-webkit-scrollbar]:hidden"
                        : "flex-wrap"
                )}
            >
            ...
            >
                ...
                <div className={cx("space-y-1 px-4 py-2")}>
                    {legendPayload.map(({ value, category, color }, index) => (
                        <div
                            key={`id-${value}-${category}-${color}-${index}`}
                    ...
                    >
                        ...
                        <Tooltip
                            ...
                            content={({ active, payload, label }) => {
                                ...
                                const stringLabel = typeof label == "number" ? label.toString() : label
                                ...
                                if (
                                    ...
                                    (active !== prevActiveRef.current ||
                                        stringLabel !== prevLabelRef.current)
                                ) {
                                    tooltipCallback({ active, payload: cleanPayload, label } as TooltipProps)
